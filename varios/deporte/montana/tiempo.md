# Precipitaciones segun el modelo GFS
Bueno para previsiones a una semana.

http://www.wetterzentrale.de/topkarten/ani/gfs/
Elegir Niederschlag. Nos muestra la evolución de las precipitaciones.
Podemos ir moviendo las horas en la barra de la izquierda


# Europa
http://www.wetterzentrale.de/topkarten/fsavneur.html
Pinchar en "Niederschlag" (precipitaciones)
Luego en una franja horaria (indica las horas), en el mapa nos pondrá para cuando estamos mirando el tiempo en la esquina superior derecha.


# Alemania
http://www.wetterzentrale.de/topkarten/ani/gfsmeur/


Niederschlagsummen
nos muestra la suma de lluvias para 24,48, ..., 144 horas




Las imagenes de europa se llaman:
http://www.wetterzentrale.de/pics/RhgfsNNNX.gif
Siendo NNN las horas a partir de este momento
X es el tipo de mapa, 3 para precipitaciones

Para europa tenemos el parte cada 6h:
00, 06, 12, ..., 240

Para la zona de alemania (comprende chamonix) cada 3h
00, 03, 06, ..., 240

# estaciones metereologicas
https://www.romma.fr/index.php
http://www.meteoclimatic.net/
  estaciones privadas en España. Ir pinchando en el mapa para ver donde están
http://www.e-metsys.fr/

https://www.wunderground.com/


# Mapa de viento
https://earth.nullschool.net/#current/wind/surface/level/orthographic=6.94,46.98,2078



# Mapas con gráficos bonitos mostrando el viento
https://www.windytv.com/45.172/5.722?2017-02-07-15,38.169,2.725,4
  GFS model
  también tiene datos históricos de estaciones cercanas
  nubes, lluvia, nieve, temperatura, etc

https://www.ventusky.com/
  modelos: ICON, GFS, GEM


http://www.avalanche-net.com/
Gráficamente buena y con bastante info
