...
class CallbackModule(CallbackBase):
    def human_log(self, data):
        try:
            if data["invocation"]["module_name"] == "command" or data["invocation"]["module_name"] == "shell":
                print("\n" + data["start"] + " -> " + data["end"] + " (" + data["delta"] + ")")
                print("\033[1m# " + data["cmd"] + "\x1b[0m")
                for l in data["stdout_lines"]:
                    print(l)
                print("\n")
        except KeyError:
            if type(data) == dict:
