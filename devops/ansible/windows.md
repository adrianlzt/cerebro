...
Y el segundo ejemplo es conectando a un windows que tiene WinRM+https pasando por un proxy https con auth.
...
windows ansible_connection=psrp ansible_become_method=runas ansible_psrp_protocol=https ansible_winrm_transport=ntlm ansible_port=5986 ansible_user=datadope ansible_host=10.0.1.5 ansible_password=SECRET ansible_psrp_cert_validation=ignore ansible_psrp_proxy=https://user:pass@10.0.0.11
```
