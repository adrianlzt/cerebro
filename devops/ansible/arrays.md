...
La idea es generar una nueva lista a partir de una lista de diccionarios, modificando los diccionarios que cumplan una condición.
Los que no cumplan la condición (el match por el id que queremos), se quedan como están.
...
vars:
  ...
  updated_list: []
tasks:
  - name: Update the list by appending to dictionaries
    set_fact:
      updated_list: "{{ updated_list + [ updated_item ] }}"
    vars:
      updated_item: "{{ (item.id == addition.id) | ternary(item | combine(addition), item) }}"
    loop: "{{ my_list }}"
