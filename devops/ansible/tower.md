...
tower-cli receive --all > awx\_$(date +%Y%m%d).json

Restaurar:
tower-cli send awx\_$(date +%Y%m%d).json
...
ansible_facts.ansible_lsb\_\_major_release:"7"
...
'template', 'file://awx**template**test_callback.j2',
'data',
json_agg(
event_data :: jsonb
order by
...
started\_\_lt=2019-12-16T18:40:59.000000%2B0100
...
api/v2/jobs/?or**status=waiting&or**status=pending&or\_\_status=new
...
<http://localhost/api/v2/inventories/> ?page_size=20&order_by=name&search=ansible_facts.ansible_lsb\_\_major_release:7
...
Hay otro sitio donde hacen referencia al ansible*facts
processJson: function(data) {
var ignored = ["type", "event_data", "related", "summary_fields", "url", "ansible_facts"];
return*.chain(data).cloneDeep().forEach(function(value, key, collection) {
...
/var/lib/awx/venv/awx/lib/python2.7/site-packages/django/core/handlers/wsgi.py(157)**call**()
...
/var/lib/awx/venv/awx/lib/python2.7/site-packages/django/utils/functional.py(35)**get**()
...
-> number = obj.get_aggregation[using, ['\_\_count']]('__count')
...
Si intento hacer:
variables\_\_jo=2 me dice que no existe el lookup "jo" para TextField
