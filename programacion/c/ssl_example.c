 ...
 *
 * Generar certificado:
 * echo -e "\n\n\n\n\n\n" | /usr/bin/openssl req -x509 -nodes -days 365 -newkey rsa:1024 -keyout cert.pem -out cert.pem
 * openssl x509 -in cert.pem -outform DER -out cert.pem.crt
 *
 * Compilar: gcc ssl.c -lssl -lcrypto
 * Arrancar server: ./a.out server
 * Arrancar cliente: ./a.out client
*/
