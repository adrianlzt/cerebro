...
DESCRIPTION
  ...
  Nagios/Icinga plugin to check a MySQL Slave if it is set to read-only...

  For security reasons use a user with as little privileges as possible. For
  example a user called $pUser:

  GRANT USAGE, PROCESS ON *.* TO '$pUser'\@'$pHost' IDENTIFIED BY 'secret';
...
my ($lName, $lValue);
$sth->bind_columns(undef, \$lName, \$lValue);
if ( $sth->err ) {
...
if ( $lValue eq  'OFF' ) {
	print("Database is read_write but should be read_only");
	exit($lError{'Critical'});
}

print "Database is read_only which is fine";
exit($lError{'OK'});
