...
DESCRIPTION
  ...
  Nagios/Icinga plugin to check difference between MySQL Master read and exec position...
...
FLAGS
  ...
  --user=name, -u       Check user for connecting to the database (default
                        $pUser).
  --criticial=#|-c      Level where critical is reached (default $pCritical)
  --warning=#|-w        Level where warning is reached (default $pWarning)
...
my ($lReadMasterLogPos, $lExecMasterLogPos);

if ( $sth->execute() ) {
	if ( my $ref = $sth->fetchrow_hashref() ) {
		$lReadMasterLogPos = $ref->{'Read_Master_Log_Pos'};
		$lExecMasterLogPos = $ref->{'Exec_Master_Log_Pos'};
	}
	else {
...
if ( ! defined($lReadMasterLogPos) ) {
	print("Read_Master_Log_Pos seems to be NULL");
	exit($lError{'Warning'});
}

if ( ! defined($lExecMasterLogPos) ) {
	print("Exec_Master_Log_Pos seems to be NULL");
	exit($lError{'Warning'});
}

my $lDelta = $lReadMasterLogPos-$lExecMasterLogPos;

if ( $lDelta >= $pCritical ) {
	print("Database execution is $lDelta bytes behind retrieving which is more than the allowed CRITICAL level of $pCritical bytes");
	exit($lError{'Critical'});
}
elsif ( $lDelta >= $pWarning ) {
	print("Database execution is $lDelta bytes behind retrieving which is more than the allowed WARNING level of $pWarning bytes");
	exit($lError{'Warning'});
}

print("Database execution is fine with $lDelta bytes behind retrieving which is less than the WARNING level of $pWarning bytes");
exit($lError{'OK'});
