...
DESCRIPTION
  ...
  Nagios/Icinga plugin to check status of MySQL Replication IO thread...
...
my ($lSlaveIoRunning, $lSlaveIoState, $lLastError, $lLastErrno);

if ( $sth->execute() ) {
	if ( my $ref = $sth->fetchrow_hashref() ) {
		$lSlaveIoRunning = $ref->{'Slave_IO_Running'};
		$lSlaveIoState   = $ref->{'Slave_IO_State'};
...
if ( $lSlaveIoRunning ne 'Yes' ) {
	print("IO thread is NOT running! Slave IO state is $lSlaveIoState, Last Errno is $lLastErrno, Last Error is $lLastError");
	exit($lError{'Critical'});
}

print("IO thread seems to run fine, Slave IO state is $lSlaveIoState");
exit($lError{'OK'});
