...
Nos mostrará el cambio aplicar (modo diff) y pedirá confirmación.

```bash
patronictl -c /etc/patroni.yml edit-config -p max_connections=101
```

Dependiendo del "context" del parámetro marcará el nodo como que necesita reinicio.
...
Para borrar un parámetro, poner igual a vacío:

```bash
patronictl -c /etc/patroni.yml edit-config -p max_connections=
```
