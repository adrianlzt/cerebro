...
Parámetros de conf para proxies activos:
  ...
  ProxyOfflineBuffer controls for how long data is kept locally if proxy can't contact server (one hour by default). Afecta al almacenamiento de datos en disco (en la db).
  ProxyLocalBuffer allows to preserve data in proxy database for later processing (Proxy will keep data locally for N hours, even if the data have already been synced with the server. This parameter may be used if local data will be used by third-party applications. (Default = 0))
