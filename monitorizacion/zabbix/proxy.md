...
Ver cuantos elementos están pendientes de envío (query costosa!):
...
--- Contar cuantos tenemos pendientes de envío (uso traducción a KB/MB para verlo mejor, pero es un número, no tamaño)
select pg_size_pretty(max(id) - (select nextid from ids where table_name='proxy_history' and field_name='history_lastid')) from proxy_history;
```

Haciendo el bruto para que deje de procesar el proxy_history (con el zabbix-proxy parado):

```sql
update ids set nextid=(select max(id) from proxy_history) where table_name='proxy_history' and field_name='history_lastid';
```
...
A partir de la 7.0.19 no se pasa siempre por el processing manager.
...
get*host_availability_data
proxy_get_hist_data
/\_get history data in batches by ZBX_MAX_HRECORDS records and stop if: */
/_ 1) there are no more data to read */
/* 2) we have retrieved more than the total maximum number of records */
/* 3) we have gathered more than half of the maximum packet size _/
proxy*get_lastid: select nextid from ids where table_name='proxy_history' and field_name='history_lastid';
...
proxy_get_hist_data
/* stop gathering data to avoid exceeding the maximum packet size\_/
