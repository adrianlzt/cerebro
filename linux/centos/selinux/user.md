...
Identity known to the SELinux policy that is mapped to users on the system, and is authorised through policy for a specific set of roles.

Para mostrar el mapeo de usuarios del sistema a selinux (necesita el paquete `policycoreutils-python-utils`):

```bash
semanage login -l
```

Mostrar usuarios de selinux (necesita tener instalado `setools-console`):
