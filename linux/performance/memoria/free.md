...
Mirar vmstat para ir sacando métricas más detalladas de memori, io, swapping, etc.

Más detalle en meminfo.md

Usar este comando, para separar entre buffers y cache (Cached and SReclaimable in /proc/meminfo):

```bash
free -hw
```

Para separar la cache en sus componentes, contenido (page cache, incluye tmpfs y shmem) y metadatos (inodos y directory entries):

```bash
grep -e ^Cached -e SReclaimable /proc/meminfo
```

Para mirar como está usando el SReclaimable usar slabtop.

Cached está explicado en proc/meminfo.md, pero, más o menos, es nr_active_file+nr_inactive_file+nr_shmem-nr_swapcached (son páginas de 4096 bytes):

```bash
grep -Ee "nr_[in]*active_file" -e nr_shmem -e nr_swapcached /proc/vmstat
```
